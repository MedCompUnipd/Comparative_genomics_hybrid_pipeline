...
if ! conda env list | grep -q "preprocessing_env"; then
    ...
    conda create -y -n preprocessing_env -c bioconda -c conda-forge \
        python=3.9 ont-fast5-api=4.1.0 pod5=0.2.0 nanoq samtools minimap2 qualimap || { echo "[ERROR] Failed to create 'preprocessing_env'."; exit 1; }
else
...
if command_exists dorado; then
    ...
    if [[ "$CURRENT_DORADO_VERSION" == "$DORADO_VERSION" ]]; then
        ...
        if [[ "$INSTALL_CHOICE" =~ ^[Yy]$ ]]; then
            ...
            mkdir -p "$DORADO_INSTALL_DIR" || { echo "[ERROR] Failed to create directory $DORADO_INSTALL_DIR"; exit 1; }
            if curl -L "https://cdn.oxfordnanopore.com/software/dorado/dorado-$DORADO_VERSION-linux-x64.tar.gz" | tar -xz -C "$DORADO_INSTALL_DIR"; then
                DORADO_BIN_PATH="$DORADO_INSTALL_DIR/dorado-$DORADO_VERSION-linux-x64/bin"
                ...
                echo "[ERROR] Failed to download or extract Dorado v$DORADO_VERSION."
                exit 1
            fi
        else
...
if ! conda env list | grep -q "kraken_env"; then
    ...
    conda create -y -n kraken_env -c bioconda -c conda-forge \
        kraken2 bracken seqtk || { echo "[ERROR] Failed to create 'kraken_env'."; exit 1; }
else
...
if ! conda env list | grep -q "qc_env"; then
    ...
    conda create -y -n qc_env -c bioconda -c conda-forge \
        nanoplot=1.40.0 quast bcftools htslib samtools mummer || { echo "[ERROR] Failed to create 'qc_env'."; exit 1; }
else
...
if ! conda env list | grep -q "assembly_env"; then
    ...
    conda create -y -n assembly_env -c bioconda -c conda-forge \
        flye wtdbg openjdk=8 paralleltask || { echo "[ERROR] Failed to create 'assembly_env'."; exit 1; }
else
...
if command_exists canu; then
    ...
    if [[ "$CURRENT_CANU_VERSION" == "$CANU_VERSION" ]]; then
        ...
        if [[ "$INSTALL_CHOICE" =~ ^[Yy]$ ]]; then
            ...
            mkdir -p "$CANU_INSTALL_DIR" || { echo "[ERROR] Failed to create directory $CANU_INSTALL_DIR"; exit 1; }
            if curl -L "https://github.com/marbl/canu/releases/download/v$CANU_VERSION/canu-$CANU_VERSION.tar.gz" | tar -xz -C "$CANU_INSTALL_DIR"; then
                CANU_BIN_PATH="$CANU_INSTALL_DIR/canu-$CANU_VERSION/bin"
                ...
                echo "[ERROR] Failed to download or extract Canu v$CANU_VERSION."
                exit 1
            fi
        else
...
if ! conda env list | grep -q "polishing_env"; then
    ...
    conda create -y -n polishing_env -c bioconda -c conda-forge \
        racon medaka bwa pilon || { echo "[ERROR] Failed to create 'polishing_env'."; exit 1; }
else
...
HIFIASM_BINARY_PATH="$HIFIASM_INSTALL_DIR/hifiasm-$HIFIASM_VERSION-Linux" # The directory containing the hifiasm executable after extraction
...
if ! command_exists hifiasm || [[ "$(hifiasm --version 2>&1 | grep -oP 'hifiasm v\K[0-9]+\.[0-9]+\.[0-9]+')" != "$HIFIASM_VERSION" ]]; then
    ...
    mkdir -p "$HIFIASM_INSTALL_DIR" || { echo "[ERROR] Failed to create directory $HIFIASM_INSTALL_DIR"; exit 1; }
    if curl -L "https://github.com/chhylp123/hifiasm/releases/download/$HIFIASM_VERSION/hifiasm-$HIFIASM_VERSION-Linux.tar.gz" | tar -xz -C "$HIFIASM_INSTALL_DIR"; then
        ...
        if [[ ! -d "$HIFIASM_BINARY_PATH" ]]; then
            echo "[ERROR] Extracted Hifiasm directory not found at $HIFIASM_BINARY_PATH. Check extraction path."
            exit 1
        fi
        echo "export PATH=\"$HIFIASM_BINARY_PATH:\$PATH\"" >> "$SHELL_PROFILE"
        source "$SHELL_PROFILE" # Reload PATH for current session
        ...
        echo "[ERROR] Failed to download or extract Hifiasm v$HIFIASM_VERSION."
        exit 1
    fi
else
...
NEXTDENOVO_BIN_PATH="$NEXTDENOVO_INSTALL_DIR/NextDenovo/bin" # The directory containing NextDenovo executable
...
if ! command_exists NextDenovo; then
    ...
    mkdir -p "$NEXTDENOVO_INSTALL_DIR" || { echo "[ERROR] Failed to create directory $NEXTDENOVO_INSTALL_DIR"; exit 1; }
    # Download to /tmp first, then unzip
    if curl -L "https://github.com/NextDenovo/NextDenovo/releases/download/v$NEXTDENOVO_VERSION/NextDenovo.zip" -o "/tmp/NextDenovo-$NEXTDENOVO_VERSION.zip"; then
        if unzip "/tmp/NextDenovo-$NEXTDENOVO_VERSION.zip" -d "$NEXTDENOVO_INSTALL_DIR"; then
            echo "[INFO] NextDenovo downloaded and extracted successfully."
            if [[ ! -d "$NEXTDENOVO_INSTALL_DIR/NextDenovo" ]]; then
                echo "[ERROR] Extracted NextDenovo directory not found. Expected at $NEXTDENOVO_INSTALL_DIR/NextDenovo."
                exit 1
            fi
            ...
            echo "[ERROR] Failed to unzip NextDenovo v$NEXTDENOVO_VERSION."
            exit 1
        fi
        rm "/tmp/NextDenovo-$NEXTDENOVO_VERSION.zip" # Clean up downloaded zip
    else
        echo "[ERROR] Failed to download NextDenovo v$NEXTDENOVO_VERSION."
        exit 1
    fi
else
...
if ! conda env list | grep -q "ndn_env"; then
    ...
    conda create -y -n ndn_env python=3.9 || { echo "[ERROR] Failed to create 'ndn_env'."; exit 1; }
else
...
if ! conda env list | grep -q "canu_env"; then
    ...
    conda create -y -n canu_env python=3.9 || { echo "[ERROR] Failed to create 'canu_env'."; exit 1; }
else
...
if ! conda env list | grep -q "fasta3_env"; then
    ...
    conda create -y -n fasta3_env python=3.9 || { echo "[ERROR] Failed to create 'fasta3_env'."; exit 1; }
else
...
if [[ "$INSTALL_PYTHON_LIBS" =~ ^[Yy]$ ]]; then
    conda activate base || { echo "[ERROR] Failed to activate 'base' environment."; exit 1; }
    pip install pandas biopython || { echo "[ERROR] Failed to install pandas or biopython."; exit 1; }
    conda deactivate
...
if ! command_exists glsearch36 || ! command_exists fasty36; then
    ...
    if [[ "$INSTALL_FASTA_PKG" =~ ^[Yy]$ ]]; then
        ...
        mkdir -p "$FASTA_DIR" || { echo "[ERROR] Failed to create directory $FASTA_DIR"; exit 1; }
        ...
        if curl -L "https://fasta.bioch.virginia.edu/fasta_tf/fasta-$FASTA_VERSION.tar.gz" | tar -xz -C "$FASTA_DIR"; then
            ...
            pushd "$FASTA_DIR/fasta-$FASTA_VERSION/src" > /dev/null || { echo "[ERROR] Failed to enter FASTA src directory."; exit 1; }
            make -f ../make/Makefile.linux64 || { echo "[ERROR] FASTA compilation failed. Please check build-essential installation and try manually."; popd > /dev/null; exit 1; }
            popd > /dev/null || { echo "[ERROR] Failed to return from FASTA src directory."; exit 1; }
            ...
            echo "[ERROR] Failed to download or extract FASTA package v$FASTA_VERSION."
            exit 1
        fi
    else
